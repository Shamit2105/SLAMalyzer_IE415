        ...
        if (vpMPs.size() > 0) {
            ...
            for (size_t j = 0; j < projectedPoints.size(); ++j) {
                ...
                if(r1.x <640 && r1.x> 0 && r1.y >0 && r1.y <480)
                    cv::circle(*pMat, cv::Point(r1.x, r1.y), 2, cv::Scalar(255, 0, 0), 1, 8);
            }
...
extern "C"
JNIEXPORT jfloatArray JNICALL
Java_com_vslam_orbslam3_vslamactivity_VslamActivity_getMapPoints(JNIEnv *env, jobject instance) {
#ifdef BOWISBIN
    if(!SLAM) return env->NewFloatArray(0);
    
    std::vector<ORB_SLAM3::MapPoint*> vpMPs = SLAM->mpTracker->mpAtlas->GetCurrentMap()->GetAllMapPoints();
    ...
    jfloatArray resultArray = env->NewFloatArray(points.size());
    if (points.size() > 0) {
        env->SetFloatArrayRegion(resultArray, 0, points.size(), &points[0]);
    }
    return resultArray;
#else
...
Java_com_vslam_orbslam3_vslamactivity_VslamActivity_getMapPointCount(JNIEnv *env, jobject instance) {
#ifdef BOWISBIN
    if(!SLAM) return 0;
    return SLAM->mpTracker->mpAtlas->GetCurrentMap()->MapPointsInMap();
#else
    return 0;
#endif
}