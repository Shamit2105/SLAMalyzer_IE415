...
import com.vslam.orbslam3.vslamactivity.obj.Ball;
import com.vslam.orbslam3.vslamactivity.obj.Cube;
import com.vslam.orbslam3.vslamactivity.obj.LinePath;
...
//      circle = new Circle(context);
        ...
        drPathLine = new LinePath();
        drPathLine.setColor(0.0f, 0.0f, 1.0f, 1.0f); // Blue
        slamPathLine = new LinePath();
        slamPathLine.setColor(1.0f, 0.0f, 0.0f, 1.0f); // Red
    }
...
//      glClear(GL_COLOR_BUFFER_BIT);
        ...
        if(flag){
            cube.draw();
            ball.draw();
            float[] mVPMatrix = MatrixState.getFinalMatrix();
            if (drPathLine != null) drPathLine.draw(mVPMatrix);
            if (slamPathLine != null) slamPathLine.draw(mVPMatrix);
        }

    }

    public void updateDrPath(List<float[]> points) {
        if (drPathLine != null) drPathLine.setPath(points);
    }

    public void updateSlamPath(List<float[]> points) {
        if (slamPathLine != null) slamPathLine.setPath(points);
    }
